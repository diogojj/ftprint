...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/19 15:51:38 by dide-jes          #+#    #+#             */
/*   Updated: 2025/11/25 16:08:12 by dide-jes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_process_format(const char format, va_list args, int char_count)
{
	if (format == 'c')
		...
		char_count += ft_checkaddres(va_arg(args, void *), char_count);
	else if (format == 'd' || format == 'i')
		char_count += ft_putnbr_fd(va_arg(args, int), 1, char_count);
	else if (format == 'u')
		char_count += ft_putnbr_base(va_arg(args, unsigned int), "0123456789",
				char_count);
	else if (format == 'x')
		char_count += ft_putnbr_base(va_arg(args, unsigned int),
...
int	ft_printf(const char *format, ...)
{
	...
	while (format[i])
	{
		if (format[i] == '%')
		{
			count += ft_process_format(format[i + 1], args, 0);
			if (count == -1)
				return (-1);
			i += 2;
		}
...
void	test_pointer(void)
{
	...
	or = printf("or: %p %p %p\n", &x, str, NULL);
	printf("Return: ft=%d, or=%d %s\n", ft, or,
		ft == or ? "\033[38;2;0;255;0m✓\033[0m" : "✗");
